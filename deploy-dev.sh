...
docker-compose -f docker-compose-dev.yml --project-name $PROJECT_NAME kill

# Видаляємо всі зупинені контейнери для проекту timecraft
docker-compose -f docker-compose-dev.yml --project-name $PROJECT_NAME rm -f

# Оновлюємо Docker-образи для проекту timecraft
docker-compose -f docker-compose-dev.yml --project-name $PROJECT_NAME pull

# Піднімаємо сервіси в фоновому режимі і будуємо образи заново
docker-compose -f docker-compose-dev.yml  --project-name $PROJECT_NAME up --build
